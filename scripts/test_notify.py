...
from src.notify.telegram import (  # noqa: E402
    TelegramError, format_signal, format_test, notify_signals, send_html,
)
...
@check("1/6 Signal JD10 — sections obligatoires + nombres FR")
def t_jd10():
    m = format_signal(JD10_SIG)
    for needle in ("🔴", "VENTE", "JD10", "Jump 10 Index", "Entrée", "62345,68",
                   "SL", "−155,7", "TP", "+467,1", "ratio 1:3", "1,00 $",
                   "79/100 (A)", "cassure M15", "CONTEXTE SYNTHÉTIQUE",
                   "tendu", "101,3", "~2,7/h", "P90 86,9", "il y a 10 min (DN)",
                   "Risque JUMP", "FAIBLE", "RSI H1 : 31,0", "aucun ordre exécuté",
                   "JD10-bearish-1725630000"):
        assert needle in m, needle
    print("   24 marqueurs présents ✅")


@check("2/6 Signal BOOM — ACHAT + échappement anti-injection")
def t_boom():
    m = format_signal(BOOM_SIG)
    for needle in ("🟢", "ACHAT", "BOOM1000", "Boom 1000 Index", "15037,08",
                   "82/100", "il y a 5 min", "−4,0 pts/h", "spikes détectés : 1"):
        assert needle in m, needle
...
@check("3/6 Message TEST — actifs/pause/seuils, sans secret")
def t_test():
...
@check("4/6 Envoi mocké — payload API + 3 chemins d'erreur")
def t_send():
...
@check("5/6 notify_signals — skip sans env, sent mocké, jamais de crash")
def t_notify():
    import urllib.request
...
@check("6/6 Wiring moteur — notify=True envoie + 📩, False silencieux (régression)")
def t_wiring():
    import tempfile
